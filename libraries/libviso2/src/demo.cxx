...
int main (int argc, char** argv) {
  ...
  for (int32_t i=0; i<373; i++) {
    ...
    try {
      ...
      cv::Mat left_img = cv::imread(left_img_file_name);
      cv::Mat right_img = cv::imread(right_img_file_name);
      ...
      uint8_t* left_img_data  = left_img.data;//(uint8_t*)malloc(width*height*sizeof(uint8_t));
      uint8_t* right_img_data = right_img.data;//(uint8_t*)malloc(width*height*sizeof(uint8_t));
      /*int32_t k=0;
