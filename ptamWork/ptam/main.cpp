...
int main(int i, char** c)
{
	...
	if(captured_frames < 2) {
	  ...
	  if(captured_frames == 2) {
	    ...
	    /*	    ghMutex = CreateMutex( 
				  ...
				  NULL);  
	    */
	    pthread_mutex_init((pthread_mutex_t*)ghMutex, NULL);
	    //start the OpenGL scene (in it's own thread)
	    DWORD tID;
	    //	    threadHandle = CreateThread(0,0,(PTHREAD_START_ROUTINE)start_opengl,0,0,&tID);
	    int err = pthread_create((pthread_t*)threadHandle, NULL, start_opengl, NULL);
  ...
  */
  ...
  if(threadHandle != NULL)
    pthread_mutex_destroy((pthread_mutex_t*)ghMutex);      
  return 0;
}
